...
import com.meneses.auth.features.user.controller.UserControllerImpl;
...
public class UserControllerTest {
    ...
    @BeforeEach
    void setUp() {
        response = new UserResponseDTO("test@email.com");
        request = new UserRequestDTO("test@email.com", Collections.singletonList("ROLE_USER"));
    }
    ...
    class FindById {
        ...
        void shouldReturnNotFound_whenUserDoesNotExist() throws Exception {
            when(userService.findById(INVALID_ID)).thenThrow(new ResourceNotFoundException("User not Found"));
    ...
    class Update {
        ...
        void shouldReturnNotFound_whenIdDoesNotExist() throws Exception {
            ...
            mockMvc.perform(put("/users/{id}",INVALID_ID)
                            .with(csrf())
                            .contentType(MediaType.APPLICATION_JSON)
                            .content(new ObjectMapper().writeValueAsString(new UserRequestDTO("test@email.com", Collections.singletonList("123")))))
                    .andExpect(status().isNotFound());
        }
    }
